...
Future<List<SearchDetailBean>> getSearchDetailApi(String WD) async {
  // 根据是否有vod_id选择API路径
  String apiPath;
  if (WD != null && WD.isNotEmpty) {
    apiPath = '${HttpConstants.SEARCH_VIDEO}$WD';
    print('获取单个视频详情,vod_id: $WD');
  } else {
  ...
  try {
    return dataList.map((item) => SearchDetailBean.fromJson(item)).toList();
  } catch (e) {
    print('数据转换失败: $e');
    return [];
  }
